...
>(nodes: Nodes, config?: { strict: Strict }) {
			...
			z.strictObject({
				...
				get input() {
					return getCompatible(
						(nodes[name] ?? (undefined as never)).input,
						scope,
					);
				},
			});
	...
	function getCompatibleDefinitions(type: $ZodType, scope?: Scope) {
		return Object.entries(nodes)
			.filter(
				([_, { output }]) => shapeOf(output, scope) === shapeOf(type, scope),
			)
	...
	function getContextCompatibilities(type: $ZodType, scope: Scope) {
		return Object.entries(scope.context ?? {})
			.filter(
				([, tokenType]) => shapeOf(tokenType, scope) === shapeOf(type, scope),
			)
	...
	function walk(type: $ZodType, scope?: Scope) {
		...
		compatibilities[shapeOf(type, scope)] = z.lazy(() =>
